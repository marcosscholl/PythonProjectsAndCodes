...
class CockpitGrid(Qt.QFrame):
    ...
    def changeAngle(self):
        ...
        if ((angle < -8.0 and self.__angle_offset < 0.0 )
            or (angle > 8.0 and self.__angle_offset > 0.0)):
...
def make():
    ...
    #CompassGrid().setWindowFlags(Qt.Qt.FramelessWindowHint)
    #CompassGrid().setAttribute(Qt.Qt.WA_TranslucentBackground, True)
    #CompassGrid().setAttribute(Qt.Qt.WA_NoSystemBackground)
    #CompassGrid().setAutoFillBackground(False)
